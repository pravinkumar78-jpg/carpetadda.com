...
export default function SearchBar({ compact = false }) {
  ...
  return (
    <div data-testid="hero-search" className={`bg-white rounded-2xl border border-black shadow-xl shadow-blue-500/10 ${compact ? "p-4" : "p-5 lg:p-7"}`}>
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="bg-transparent rounded-xl p-0 h-auto mb-5 inline-flex gap-2">
          {[["sale", "Buy"], ["rent", "Rent"], ["new_projects", "Projects"]].map(([v, l]) => (
            <TabsTrigger key={v} value={v} data-testid={`tab-${v}`} className="rounded-lg px-4 py-2 text-sm font-medium bg-[#708DE6] text-white hover:bg-[#5C76D4] data-[state=active]:bg-[#708DE6] data-[state=active]:text-white data-[state=active]:shadow-md data-[state=active]:font-semibold transition-colors">
              {l}
            </TabsTrigger>
          ))}
        </TabsList>
