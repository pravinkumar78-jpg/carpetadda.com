...
function AdminPasswordChange() {  const [f,setF]=useState({current_password:"",new_password:"",confirm_password:""}); const [busy,setBusy]=useState(false);
  ...
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><Input required type="password" placeholder="Current password" value={f.current_password} onChange={e=>setF({...f,current_password:e.target.value})}/><Input required minLength={8} type="password" placeholder="New password" value={f.new_password} onChange={e=>setF({...f,new_password:e.target.value})}/><div className="flex gap-2"><Input required minLength={8} type="password" placeholder="Confirm new password" value={f.confirm_password} onChange={e=>setF({...f,confirm_password:e.target.value})} onKeyDown={e=>e.key==="Enter"&&submit(e)}/><button type="button" onClick={submit} disabled={busy} className="px-4 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium disabled:opacity-60">{busy?"Saving…":"Update"}</button></div></div>;
}
