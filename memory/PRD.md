...
## Implemented (2026-08-24 — filter chips + project mobile bar + deploy prep)
- FilterChips.jsx (new shared presentational component): removable chips for applied filters on Properties + Projects listings; removal calls the page's existing update(key,"") → deletes URL param → existing refetch; locked/page params excluded (category on dedicated pages, sort, page); friendly labels (Rent/Buy, "2 BHK", Min/Max ₹, Verified only, etc.)
...
## Implemented (2026-08-27 — approval flow hardening + hero arrows + RERA QR)
1. Approval flow (mostly pre-existing; VERIFIED: agent submission → pending_review → admin pending list; self-publish blocked; live-listing edits staged to Approvals; admin listings go live directly). NEW gap closed: account approval — User model gained `approved` (None/True = approved; False = pending). Self-registered agent/developer/owner → approved=False → blocked from creating properties/projects (403 "awaiting admin approval"); admin-created users pre-approved; existing accounts (field absent) unaffected. Admin → Users gained a "Pending approval" filter chip (count badge) + "Approved" checkbox in the user edit dialog
2. Project Detail hero: carousel prev/next arrows now render only when gallery has >1 image (single main image → no arrows)
3. RERA: form entry editor now offers ONLY "Upload RERA QR Image" + "RERA QR Link" (certificate upload input removed; existing certificate_url data preserved and still rendered). Detail page shows exactly ONE QR image — RERA QR Link wins when valid (http or / path), uploaded image otherwise, onError falls back to the upload; duplicate "official page" link row removed
- Verified: register→pending→403→approve→pending_review→404-until-approved chain via curl; seed accounts unaffected; arrows 0/0; link-priority + fallback tested live on lodha-opulis-dombivli (data restored after); form labels confirmed visually (CSS-uppercase labels). REDEPLOY needed for production

## Implemented (2026-08-28 — visitor information disclaimer)
- VisitorDisclaimer (App.js, route-aware): shows once per session (sessionStorage eh_disclaimer_ok=version) on any listing/detail route (/properties, /projects, /commercial-properties, /new-launch, /rtmi, /property/*, /project/*); "I Understand" (or any dismiss) acknowledges and never re-prompts that session; browsing never blocked
- Ack record: POST /api/disclaimer/ack (public, best-effort) → db.disclaimer_acks via DisclaimerAck model — visitor_id (persistent localStorage uuid), user_id when logged in (current_user_optional), version, acknowledged, timestamps
- Verified: appears on first listing view, closes on I Understand, listings browsable, no repeat across pages in session, DB records confirmed for anonymous + authenticated visitor; QA acks cleaned. REDEPLOY needed for production
- STORAGE HARDENING (same session): put_object (storage.py) no longer writes to pod-local disk when durable object storage is enabled — remote is the sole write target (local write remains only as a no-key dev fallback; get_object still lazy-populates the read cache). Verified: upload→serve 200, existing images serve, QA file cleaned. This also cleared the ephemeral-upload-storage deployment lint flags

## Implemented (2026-08-29 — Admin Visitor Reports)
- No prior analytics existed → added minimum tracking: lib/track.js (anonymous visitor_id in localStorage + per-tab session_id; fire-and-forget, never blocks) + AnalyticsTracker in App.js (page_view on public route changes only — staff areas excluded; property_view/project_view derived from URL; global click listener captures wa.me/tel: clicks)
...
## Implemented (2026-08-31 — units hidden, Add City, address autocomplete w/ auto-fill)
1. Total Units removed from project LISTING cards (ProjectCard grid+list variants; detail page + data untouched)
2. City field (PropertyForm + ProjectForm): dropdown keeps existing cities + new "Other — Add City" option revealing a manual text input (slugified); saved custom cities prefill correctly on edit
3+4. Address search (Option B — existing OSM/Photon provider, no Google key): new GET /api/geo/search (Photon proxy, returns label/city/locality/lat/lng) + shared AddressSearchInput component on both forms' Address fields; on select → address, lat/lng, Google Maps link (existing google_map_link field) always set; city set when provided (known → dropdown value, unknown → manual input); locality filled ONLY when empty (manual values never overwritten)
- Verified: geo endpoint returns structured results; property form — suggestion select auto-filled address/lat/lng/map-link and switched city to Other+"dombivali"; project form — BKC search set city=Mumbai (dropdown match), lat/lng, map link, locality default preserved; project listing cards show no units; existing listings render. No test data saved. REDEPLOY needed for production

## Investigation (2026-08-31 — reported production 404 on homepage)
- User's mobile screenshot showed carpetadda.com rendering the app's 404 page. Live verification on production (mobile 390px + fresh cache-busted reloads): https://carpetadda.com AND https://www.carpetadda.com BOTH load the homepage correctly (server 200, www → non-www redirect works, hero/search/ticker render, zero 404, no console errors). Preview route table intact (Route path="/" → Home at App.js:58).
- Conclusion: transient deploy-window state (stale HTML + new chunks mid-deploy) or device cache — NOT a code bug. NO code change made (any change would have been unjustified). If it recurs outside a deploy window, advise hard refresh; if persistent → Emergent Support (CDN/domain config is platform-side).

## Fix (2026-09-01 — agent leads tab, image persistence chain, card image fallback)
1. Agent Leads: already present in Agent Workspace (Leads tab, scoped via /leads agent filter) — verified in browser: agent sees ONLY their 3 listing leads with statuses/follow-ups; no admin data. Production simply needs REDEPLOY (older build lacked it)
2. Image persistence: traced entire chain via API — agent upload (/uploads → durable object storage) → create pending_review → agent edit → admin pending view → approve → published: images intact at EVERY step incl. staged live-edit apply. Real gap found+fixed: AdminApprovals diff view SKIPPED images/main_image → admin now sees submitted-image thumbnails in the Pending Changes cell (data was always applied on approve; only the VIEW hid it). Note: production images lost pre-2026-08-28 were due to pod-local storage — fixed by durable storage; redeploy required
3. Card image mapping: PropertyCard + ProjectCard now use main_image || images[0] (same source as detail pages) with onError fallback (tries the other images, then hides gracefully) — fixes cards whose listing has main_image but empty/different images array (e.g. production "Birla Vanya")
- Verified: agent leads UI, full image chain incl. staged approve, admin thumbnails visible, homepage 16 card images 0 broken on mobile. QA listing archived, test file cleaned. REDEPLOY needed
...
## Verified (2026-09-03 — List Property auth gate flow; NO code change needed)
- Request: List Property must redirect logged-out users to Login/Register first, then to the Property Listing form after auth. Investigation showed the flow already exists (PostProperty.jsx:22 Navigate to /login?next=/post-property; AuthPages login+register honor ?next= and cross-link preserving it; Header desktop+mobile + Home CTA all point to /post-property)
- Browser verification (fresh logged-out sessions): (1) header List Property → /login?next=/post-property, form NOT rendered; (2) login→Create account link carries next to /register; (3) fresh registration → lands on /post-property with form; (4) Home "List Property Free" CTA → same login redirect; (5) login as existing user → lands on /post-property with form. ALL PASS. Test register account cleaned up via admin delete
- Conclusion: zero code changes made. If the user saw the form open directly on production, it's an older build — REDEPLOY production

## Fixed (2026-09-03 — Properties Listing Call/WhatsApp route to assigned agent)
...
## Implemented (2026-09-03 — Project gallery multi-upload + original aspect ratio)
- ImageUpload.jsx: new OPT-IN props only — `multiple` (+ onMultiple: files upload sequentially via the existing /admin/uploads flow, one callback with all URLs in picker order) and `contain` (preview object-contain instead of object-cover). Defaults unchanged for every other usage
...
## Fixed (2026-09-04 — Properties menu click freeze: Radix modal-dropdown body-lock race)
...
## Implemented (2026-09-05 — Smart Loan Calculator system)
- ONE reusable component frontend/src/components/SmartLoanCalculator.jsx — 3 tabs: Home Loan EMI (amount/down payment/rate/tenure → EMI, principal, interest, total; standard amortisation), Eligibility (income/existing EMIs/rate/tenure/own contribution → eligible loan @50% FOIR, property budget, est. EMI, repayment capacity; labelled estimate-not-sanction), Mortgage/LAP (property value/existing loan/rate/tenure → loan @~60% LTV, EMI, interest, total repayment). Sliders + editable numeric inputs, live results
- Placements (same component): Home page section above Subscribe; bottom of every Property Detail (defaults to p.price||p.rent) and Project Detail (defaults to p.price_from) — editable, CTAs carry property/project context; new /loan-calculator page + "Loan Calculator" menu link (desktop+mobile); legacy /emi-calculator preserved, now renders the shared component
- CTA "Get Loan Assistance" reuses existing /home-loan lead form (HomeLoan now also reads loan_type param). SEO: /loan-calculator in SeoManager + AdminSeo
- Verified: EMI math exact (50L@8.5%/20y → ₹43,391; 1Cr → ₹86,783); eligibility ₹57.62L on 1L income; LAP ₹60L on 1Cr value; property/project price defaults + context CTAs; mobile responsive, no overflow; existing pages unchanged. REDEPLOY needed for production

## Implemented (2026-09-06 — Property form new "Price" tab)
- New dedicated Price tab (between Details and Location) in the Property Listing form: Property Cost / Sale Price moved here from Basic (rent listings show Monthly Rent + Deposit here too), Stamp Duty Rate %, Registration Amount ₹, Society Charges ₹ with note "GST amount is not included in Society Charges.", Society Maintenance Rate ₹/sq.ft., City Maintenance Applicable? Yes/No → Monthly City Maintenance Rate ₹/sq.ft. + City Maintenance Months + live read-only total = existing Carpet Area × rate × months (exact Indian-formatted ₹, e.g. 850 × 2.5 × 12 = ₹25,500)
...
## Changed (2026-09-06 — Loans dropdown nav + Smart Calculator beside Loans form)
